...
trait WikipediaApi {
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
     ...
     */
    def timedOut(totalSec: Long): Observable[T] = obs.timestamp.filter(_._1 <= totalSec).map(_._2)
     ...
     */
    def concatRecovered[S](requestMethod: T => Observable[S]): Observable[Try[S]] = obs.flatMap(requestMethod).map(Try(_))
