...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  % This is the only function that you're expected to change. Right now, it just returns a lot of zeros, which is obviously not the correct output. Your job is to replace that by a correct computation.
   hid_input = model.input_to_hid * data.inputs; % <number of hidden units> by <number of data cases>
   hid_output = logistic(hid_input); % <number of hidden units> by <number of data cases>
   class_input = model.hid_to_class * hid_output; % <number of classes, i.e. 10> by <number of data cases>
   ...
   class_prob = exp(log_class_prob); %<number of classes, i.e. 10> by <number of data cases>
  
   ret.input_to_hid = ((class_prob - data.targets)' * model.hid_to_class .* (hid_output  - hid_output.^2)')' * data.inputs' / size(data.targets, 2) + model.input_to_hid * wd_coefficient;
   ret.hid_to_class =  (class_prob - data.targets) * hid_output' / size(data.targets, 2) + model.hid_to_class * wd_coefficient;
end
